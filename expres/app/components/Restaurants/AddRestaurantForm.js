...
export default function AddRestaurantForm(props) {
  ...
  const addRestaurant = () => {
    if (!restaurantName || !restaurantAddress || !restaurantDescription) {
      ...
      toastRef.current.show("Tienes que localizar al restaurante en el mapa");
    } else {
      setIsLoading(true);
      uploadImagesStorage(imagesSelected).then((arrayImages) => {
        db.collection("restarurants")
          .add({
            name: restaurantName,
  ...
  return (
    <ScrollView>
      ...
      <Button
        ...
        buttonStyle={styles.btnAddresRestaurant}
      />
      <Map
...
function UploadImagen(props) {
  ...
  const imageSelect = async () => {
    ...
    if (resultPermissionCamera === "denied") {
      ...
      if (result.cancelled) {
        toastRef.current.show(
          "Has cerrado la galeria sin seleccion ninguna imagen",
          2000
        );
      } else {
